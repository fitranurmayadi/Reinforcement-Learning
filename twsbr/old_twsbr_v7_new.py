...
class TwsbrEnv(gym.Env):
    ...
    def _init_physics_client(self):
        ...
        if self.render_mode == "human":
            ...
            start_position, start_orientation = [0.0, 0.0, 0.0], pybullet.getQuaternionFromEuler([0, 0, random.uniform(-np.pi/8, np.pi/8)])
    ...
    def step(self, action):
        ...
        terminated = True if abs(observation[2]) >= 0.25 or abs(observation[3]) >= 0.95 else False #or or 
    ...
    def _get_obs(self):
        ...
        if linear_speed < self.target_speed:
            self.target_pitch += 0.005
        elif linear_speed > self.target_speed:
            self.target_pitch -= 0.005
    ...
    def _get_reward(self):
        # Ambil observasi
        target_pitch, target_speed, pitch, yaw, linear_speed, linear_left_wheel, linear_right_wheel, left_motor_power, right_motor_power,  left_wheel_revolutions, right_wheel_revolutions, prev_target_pitch, prev_target_speed, prev_pitch, prev_yaw, prev_linear_speed, prev_linear_left_wheel, prev_linear_right_wheel, prev_left_motor_power, prev_right_motor_power, prev_left_wheel_revolutions, prev_right_wheel_revolutions = self.observation_from_step
        
                   
        
        target_revolutions = 1.0
        target_pitch = 0.025  # 4.55 degree
        target_speed = 0.03  # 0.5 m/s
        ...
        reward -= ((target_pitch - pitch) **2) * 4
        reward -= (abs(yaw) **2) * 2
        reward -= ((target_speed - linear_speed) ** 2) * 1 

        #reward -= (abs(target_speed - linear_speed) **2)
        #reward -= 0.0 if abs(left_motor_power) < 0.5 else abs(left_motor_power * 10) **2
        #reward -= 0.0 if abs(right_motor_power) < 0.5 else abs(right_motor_power * 10) **2
        #kompensasi stabil


        reward += 0.05 if abs(pitch) < 0.1 else -0.0
        reward += 0.05 if abs(yaw) < 0.25 else -0.0
        reward += 0.05 if current_revolutions >= previous_revolutions and left_wheel_revolutions >= 0 and right_wheel_revolutions >= 0 else -0.0
        reward += 0.1 if abs(pitch) < 0.035 and abs(yaw) < 0.25 and current_revolutions >= previous_revolutions and left_wheel_revolutions >= 0 and right_wheel_revolutions >= 0 else -0.0 
        reward += 0.1 if abs(pitch) < 0.05 and abs(yaw) < 0.5 and current_revolutions >= previous_revolutions and left_wheel_revolutions >= 0.01 and right_wheel_revolutions >= 0.01 else -0.0
        reward += 0.1 if abs(pitch) < 0.05 and abs(yaw) < 0.5 and current_revolutions >= previous_revolutions and left_wheel_revolutions >= 0.01 and right_wheel_revolutions >= 0.01 else -0.0
        ...
        if self.render_mode == "hun":        
            print(f"Pitch: {pitch:.4f},"
            f"Yaw: {yaw:.4f}, "
            f"Linear Speed: {linear_speed:.4f}, "
            f"Linear Left Wheel: {linear_left_wheel:.4f}, "
            f"Linear Right Wheel: {linear_right_wheel:.4f}, "
            f"Left Wheel Power : {left_motor_power:.4f}, "
            f"Right Wheel Power : {right_motor_power:.4f}, "
            f"Left Wheel Revolutions: {left_wheel_revolutions:.4f}, "
            f"Right Wheel Revolutions: {right_wheel_revolutions:.4f}, "
            
            f"Previous Pitch: {prev_pitch:.4f}, "
            f"Previous Yaw: {prev_yaw:.4f}, "
            ...
            f"Previous Left Wheel Power : {prev_left_motor_power:.4f}, "
            f"Previous Right Wheel Power : {prev_right_motor_power:.4f}, "
