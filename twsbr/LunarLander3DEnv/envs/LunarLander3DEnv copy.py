...
class LunarLander3DEnv(gym.Env):
    ...
    def _get_reward(self, obs, action):
        ...
        reward -= 0.001 * ((action[0]+ 1) / 2) **2
        squared_actions = ((action[1:] + 1) / 2) ** 2  # Kuadratkan setiap elemen secara individual
        reward -= 0.0001 * np.sum(squared_actions)  # Jumlahkan hasilnya dan kurangkan dari reward
