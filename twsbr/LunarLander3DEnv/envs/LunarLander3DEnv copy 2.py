...
class LunarLander3DEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, max_thrust=3200.0, truncation_steps=5000, drag_coeff=0.5, wind_force=15, wind_freq=0.1, planet="earth"):
    ...
    def _init_physics_client(self):
        ...
        # Contoh: kondisi awal semakin diacak seiring naiknya difficulty
        base_altitude = 100
        # Semakin tinggi difficulty, semakin besar randomisasi posisi dan orientasi
        start_x = random.uniform(-100 * diff, 100 * diff)
        start_y = random.uniform(-100 * diff, 100 * diff)
    ...
    def _get_reward(self, obs, action):
        ...
        # Reward shaping: semakin kecil error, semakin besar reward-nya.
        shaping = ( 0 )

        # Diferensial shaping: reward diberikan berdasarkan perubahan shaping dari langkah sebelumnya
        ...
        reward = (
            (10 - 5 * np.linalg.norm(target_pos - pos)**2)
            +(10 - 50 * np.linalg.norm(target_vel - lin_vel)**2)
            +(10 - 500 * np.linalg.norm(orient)**2)
            +(1 - 1 * np.linalg.norm(ang_vel)**2)
        )
        
        # Penalti untuk penggunaan aksi
        reward -= 0.1 * np.abs(action[0]) **2
        squared_actions = action[1:] ** 2  # Kuadratkan setiap elemen secara individual
        reward -= 0.01 * np.sum(squared_actions)  # Jumlahkan hasilnya dan kurangkan dari reward

        # Penalti jika terjadi crash
        crash_condition = ( 
                abs(pos[0]) > 1 or
                abs(pos[1]) > 1 or
                pos[2] < 0 or pos[2] > 1 or 
                np.abs(orient[0]) > 0.75 or
                np.abs(orient[1]) > 0.75 or
                np.abs(orient[2]) > 0.75 or
        ...
        landing_condition = (
                np.sum(contact > 0.0) >= 2 and
                pos[2] < 0.01 and
                np.linalg.norm(lin_vel) < 0.1
        )
        if landing_condition:
                reward += 500 * np.sum(contact > 0.0)
                return reward, True, False  # terminated=True, truncated=False
    ...
    def render(self):
        if self.render_mode == "human":
            ...
            self._bullet_client.resetDebugVisualizerCamera(cameraDistance=25,
                                                             cameraYaw=45,
                                                             cameraPitch=-45,
