...
class TwsbrEnvV2(gym.Env):
    ...
    def __init__(self,
        ...
        super(TwsbrEnvV2, self).__init__()
        ...
        self.state_limit = np.array([
            ...
            np.pi,           # Yaw (error posisi garis): ±4 unit
    ...
    def step(self, action):
        ...
        terminated = True if abs(observation[0]) >= 0.25 or abs(observation[1]) >= 0.75 or abs(observation[2]) >= 0.95 else False #or or 
    ...
    def _get_first_obs(self):
        ...
        yaw = line 
        obs = np.array([pitch, yaw, linear_speed, linear_left_wheel, linear_right_wheel, self.left_motor_power, self.right_motor_power, left_wheel_revolutions, right_wheel_revolutions])
        return obs
    ...
    def _get_obs(self):
        ...
        line = self._get_current_line_position()
        yaw = line
        obs_temp = np.array([pitch, yaw, linear_speed, linear_left_wheel, linear_right_wheel, self.left_motor_power, self.right_motor_power, left_wheel_revolutions, right_wheel_revolutions])
    ...
    def _get_reward(self):
        ...
        reward += 0.1 if abs(yaw) < 0.1 else -0.03
        reward += 0.1 if current_revolutions >= previous_revolutions and left_wheel_revolutions >= 0 and right_wheel_revolutions >= 0 else -0.03
